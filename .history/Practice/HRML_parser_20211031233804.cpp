...
struct Node {
  Node(Node * parent,string tag):parent()
  Node *parent = nullptr;
  string tag;
};
void ParseData(unordered_map<string, vector<pair<string, string>>> &dict,
               Node &new_tag, vector<Node *> &hierarchy, Node *current,
               int &n) {
  string line;
  getline(cin, line);
  ...
  if (line[1] != '/') {
    ...
    current = &new_tag;
    hierarchy.emplace_back(new new_tag);
    // cout<<"alive\n";
...
int main() {
  ...
  for (int n = 0; n < N; n++) {
    Node node;
    ParseData(tag_dict, node, tag_hierarchy, current, n);
