...
void ExpandNeighbors(vector<vector<int>> &node,vector<vector<int>> &open, vector<vector<State>> &grid, int goal[2]){
...
vector<vector<State>> Search(vector<vector<State>> board, int start[2],
  ...
  while (!openList.empty()) {
    ...
    for (auto neighbor : deltas) {
      int deltax = x + neighbor[0];
      int deltay = y + neighbor[1];
      if (CheckValidCell(deltax, deltay, board))
        AddToOpen(x, y, g + 1, Heuristic(deltax, deltay, goal[0], goal[1]),
                  openList, board);
    }
  }
